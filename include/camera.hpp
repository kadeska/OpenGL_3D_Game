...
// Raycast function to find the first occupied block the camera is looking at
std::optional<glm::ivec3> pickBlock(const glm::vec3& camPos, const glm::vec3& camDir, float maxDistance, const World& world)
{
    glm::vec3 pos = camPos;
    float step = 0.1f; // step size, smaller = more precise but slower
